...
class App(ctk.CTk):
    ...
    def build_processed_panel(self):
        ...
        self.html_label = HTMLLabel(self.html_wrapper, background="#1d1e1e", foreground="#ffffff", html="<body style='background-color: #1d1e1e; color: #ffffff;'><h3>Processed Markdown Output will appear here...</h3></body>")
    ...
    def toggle_markdown_view(self):
        ...
        if self.is_rendered_markdown:
            ...
            # Convert text to markdown html with dark theme wrapper styling and explicit white text styling
            text_content = self.processed_textbox.get("1.0", "end-1c")
            raw_html = markdown.markdown(text_content)
            html_content = f"<html><head><style>body {{ background-color: #1d1e1e; color: #ffffff; font-family: sans-serif; }} * {{ color: #ffffff !important; }}</style></head><body style='background-color: #1d1e1e; color: #ffffff;'>{raw_html}</body></html>"
    ...
    def send_prompt(self):
        ...
        if self.is_rendered_markdown:
            self.html_label.set_html(f"<html><head><style>body {{ background-color: #1d1e1e; color: #ffffff; font-family: sans-serif; }} * {{ color: #ffffff !important; }}</style></head><body style='background-color: #1d1e1e; color: #ffffff;'>{markdown.markdown(self.processed_textbox.get('1.0', 'end-1c'))}</body></html>")
